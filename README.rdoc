...
To get the console running, you'll need to do the following. First, create a
client application for it, with the redirect URI pointing to where you expect
to mount the console, e.g. "http://example.com/oauth/admin").

Next, in your application, make sure to only authorize administrators with
access to the console, which requires scope +oauth-admin+. For example:
...
Make sure you do that, or you'll allow anyone access to the OAuth console.

Next, mount the OAuth console as part of your application, and feed it the

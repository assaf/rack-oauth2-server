...
Rails 2.x and later, place this in config/environment.rb:

  config.middleware.use Rack::OAuth2::Server

For Sinatra/Padrino, add this in your application:
...
- :authorize_path -- Set this if you don't care for the default
  /oauth/authorize.
- :authorization_types -- Supported authorization types are authorization code
  (code) and access token (token). Set to code or token if you want one of these
  but not both.
...
- :scopes -- Array listing all allowed scopes, space separated list of scopes,
  or nil if you don't care for scopes.

Sammy("#main", function(app) {
  ...
  var clients = null;
  // Invokes callback with the client specified by identifier.
  function withClient(id, cb) {
    if (clients) {
      var client = _.detect(clients, function(c) { return c.id == id });
      if (client)
        cb(client);
    } else {
  ...
  this.get("#/", function(context) {
    ...
    $.getJSON(api + "/clients", function(json) {
      clients = json.list;
      context.partial("admin/views/clients.tmpl", { clients: clients, tokens: json.tokens });
    });
  });
  // View single client
  this.get("#/client/:id", function(context) {
    withClient(context.params.id, function(client) {
      $.getJSON(client.url, function(client) {
  ...
  this.get("#/client/:id/:page", function(context) {
    withClient(context.params.id, function(client) {
      $.getJSON(client.url + "?page=" + context.params.page, function(client) {
        context.title(client.displayName);
        context.partial("admin/views/client.tmpl", client)
      })
    });
